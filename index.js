...
app.use(express.static(path.join(__dirname, 'public')))
    ...
    .get('/postalCalc', (req, res) => res.render('pages/index'))
...
function handlePostalRate(req, res) {
    ...
    var result;
    switch (String(type)) {
        case "letters-stamped"://Letters (Stamped)
            if (weight > 3) {
                result = 1.00;
            } else {
            ...
            break;
        case "letters-metered"://Letters (Metered)
            if (weight > 3) {
                result = 0.95;
            } else {
            ...
            break;
        case "large-env"://Large Envelopes (Flats)
            result = 1 + (Math.floor(weight - 1) * .15);
            break;
        case "first-class"://First-Class Package Service—Retail
            if (weight > 12) {
                result = 5.71;
            } else if (weight >= 8) {
                result = 5.19;
